...
import { a4 as Ln, S as St, a5 as Le, a as $, $ as $e, Q as Qe$1, z as ze, b as be$1, I as Ie$1, A as At, m as mt, h as Rt, L as f, U as dt, _ as _e, W as Oe, Z as Me, c as ct, O as Te, n as Hi, q as Vi, v as vn, s as yn, x as xn, N as Nn, o as Q, i as ie, p as Ee, t as Xt, w as wn, a1 as qi, a6 as ro, a2 as qe, a3 as it } from '../nitro/nitro.mjs';
...
import { b as be, v as ve, N as Ne$1, p as pe, l as le } from './collapsible-BJOti_Mx.mjs';
...
const Ke = cva("relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground", { variants: { variant: { default: "bg-background text-foreground", destructive: "border-destructive/50 text-destructive dark:border-destructive [&>svg]:text-destructive", warning: "border-amber-500/50 text-amber-700 [&>svg]:text-amber-700" } }, defaultVariants: { variant: "default" } }), Ie = j.forwardRef(({ className: l, variant: x, ...m }, v) => jsx("div", { ref: v, role: "alert", className: f(Ke({ variant: x }), l), ...m }));
Ie.displayName = "Alert";
const Xe = j.forwardRef(({ className: l, ...x }, m) => jsx("h5", { ref: m, className: f("mb-1 font-medium leading-none tracking-tight", l), ...x }));
Xe.displayName = "AlertTitle";
const je = j.forwardRef(({ className: l, ...x }, m) => jsx("div", { ref: m, className: f("text-sm [&_p]:leading-relaxed", l), ...x }));
je.displayName = "AlertDescription";
function Qe({ project: l, onProjectUpdated: x, trigger: m, userId: v }) {
  const M = useNavigate(), C = useSetAtom(St), [b, c] = useState(false), [h, w] = useState(false), [j, P] = useState(false), [D, y] = useState(false), [a, k] = useState(""), [o, n] = useState({ name: "", description: "", isPrivate: false }), [i, r] = useState(true);
  useEffect(() => {
    var _a;
    if (b) {
  ...
  const d = (g) => {
    ...
    w(true);
    try {
      const { data: s } = await Te().chatrooms.updateProject(l.id, { teamspaceId: l.teamspaceId, context: l.context, creatorId: v, name: o.name, description: o.description, isPrivate: o.isPrivate, members: o.isPrivate ? [] : (_a = l.members) == null ? void 0 : _a.map((f) => ({ projectId: l.id, userId: f.user.id, role: f.role, invitedByUserId: v })) });
    ...
    P(true);
    try {
      await Te().chatrooms.deleteProject(l.id), toast.success("Project deleted successfully"), c(false), l.teamspaceId ? (C((g) => g.map((f) => ({ ...f, projects: f.projects.filter((I) => I.id !== l.id) }))), M({ to: "/teamspace/$teamspaceId", params: { teamspaceId: l.teamspaceId } })) : M({ to: "/chat" });
    } catch (g) {
  ...
  return jsxs(Hi, { open: b, onOpenChange: c, children: [jsx(Vi, { asChild: true, children: m || jsxs($, { size: "sm", variant: "outline", className: "hover:cursor-pointer", children: [jsx(Settings, { className: "h-4 w-4 mr-2" }), " Project Settings"] }) }), jsx(vn, { className: "sm:max-w-[500px]", children: D ? jsxs("div", { children: [jsxs(yn, { children: [jsx("div", { className: "h-1.5 bg-red-500 w-full absolute top-0 left-0 rounded-t-lg" }), jsxs(xn, { className: "text-xl mt-2 text-red-600 flex items-center", children: [jsx(Trash2, { className: "h-5 w-5 mr-2" }), " Delete Project"] }), jsx(Nn, { children: "This action cannot be undone. This will permanently delete the project and remove all associated chats and data." })] }), jsxs("div", { className: "py-4", children: [jsx("div", { className: "bg-red-50 border border-red-200 rounded-md p-4 mb-4", children: jsxs("div", { className: "flex items-center", children: [jsx("span", { className: "text-xl mr-2", children: "\u{1F4A9}" }), jsxs("p", { className: "text-sm text-red-800", children: ["Are you absolutely sure you want to delete ", jsx("strong", { children: l.name }), "? This action is irreversible!"] })] }) }), jsxs("div", { className: "grid gap-2", children: [jsxs(Q, { htmlFor: "confirmName", className: "text-sm font-medium", children: ["To confirm, type ", jsx("span", { className: "font-bold", children: l.name }), " below:"] }), jsx(ie, { id: "confirmName", value: a, onChange: (g) => k(g.target.value), placeholder: `Type "${l.name}" to confirm` })] }), jsxs("div", { className: "flex justify-between mt-6", children: [jsx($, { type: "button", variant: "outline", onClick: () => y(false), disabled: j, children: "Cancel" }), jsx($, { type: "button", variant: "destructive", className: "bg-red-600 hover:bg-red-700", onClick: L, disabled: a !== l.name || j, children: j ? "Deleting..." : "Permanently Delete Project" })] })] })] }) : jsxs("form", { onSubmit: S, children: [jsxs(yn, { children: [jsx("div", { className: "h-1.5 bg-emerald-500 w-full absolute top-0 left-0 rounded-t-lg" }), jsx(xn, { className: "text-xl mt-2", children: "Project Settings" }), jsx(Nn, { children: "Update the details and settings of your project" })] }), jsxs("div", { className: "grid gap-4 py-4", children: [jsxs("div", { className: "grid gap-2", children: [jsxs(Q, { htmlFor: "name", children: ["Project Name ", jsx("span", { className: "text-red-500", children: "*" })] }), jsx(ie, { id: "name", name: "name", value: o.name, onChange: d, placeholder: "e.g. Marketing Campaign", required: true })] }), jsxs("div", { className: "grid gap-2", children: [jsx(Q, { htmlFor: "description", children: "Description" }), jsx(Ee, { id: "description", name: "description", value: o.description, onChange: d, placeholder: "What is this project about?", rows: 3 })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Xt, { id: "isPrivate", checked: o.isPrivate, onCheckedChange: T, disabled: !i && !l.isPrivate }), jsxs("div", { className: "grid gap-1.5 leading-none", children: [jsxs(Q, { htmlFor: "isPrivate", className: `text-sm font-medium leading-none flex items-center ${!i && !l.isPrivate ? "text-muted-foreground" : ""}`, children: [jsx(Lock, { className: "h-3.5 w-3.5 mr-1.5 text-muted-foreground" }), "Make project private"] }), jsx("p", { className: "text-xs text-muted-foreground", children: "Private projects are only visible to invited members" })] })] }), !i && !l.isPrivate && jsxs(Ie, { variant: "warning", className: "bg-amber-50 text-amber-800 border-amber-200", children: [jsx(AlertCircle, { className: "h-4 w-4" }), jsx(je, { children: "Projects with multiple members cannot be made private" })] }), jsx("div", { className: "border-t pt-4 mt-2", children: jsxs("div", { className: "flex flex-col space-y-2", children: [jsx("h3", { className: "text-sm font-medium text-red-600", children: "Danger Zone" }), jsx("p", { className: "text-xs text-muted-foreground", children: "Once you delete a project, there is no going back. This action cannot be undone." }), jsxs($, { type: "button", variant: "destructive", className: "mt-2 bg-red-600 hover:bg-red-700", onClick: () => y(true), children: [jsx(Trash2, { className: "h-4 w-4 mr-2" }), " Delete Project"] })] }) })] }), jsxs(wn, { children: [jsx(qi, { asChild: true, children: jsx($, { variant: "outline", type: "button", disabled: h, children: "Cancel" }) }), jsx($, { type: "submit", className: "bg-emerald-600 hover:bg-emerald-700", disabled: h, children: h ? "Saving..." : "Save Changes" })] })] }) })] });
}
  ...
  const j = useFileUpload({ maxFiles: 10, maxFileSize: 52428800 }), [P, D] = useState(false), [y, a] = useState(false), k = async (n) => {
    if (n.length) {
      ...
      try {
        ...
        const d = await Te().file_management.uploadFilesWithMetadata("POST", i);
...
function Ye({ members: l }) {
  ...
  return jsxs("div", { className: "space-y-3", children: [jsx("div", { children: D.map((a) => {
    ...
    return jsx("div", { className: "py-1.5", children: jsxs("div", { className: "relative flex items-center justify-between group", children: [jsxs("div", { className: "flex items-center gap-2", children: [jsx(qe, { className: "h-7 w-7", children: ((_a = a.user) == null ? void 0 : _a.image) ? jsx("img", { src: a.user.image, alt: ((_b = a.user) == null ? void 0 : _b.name) || "User" }) : jsx("div", { className: "bg-gray-200 h-full w-full flex items-center justify-center text-gray-700 text-xs", children: (((_c = a.user) == null ? void 0 : _c.name) || "U").charAt(0) }) }), jsx(dt, { children: jsxs(_e, { children: [jsx(Oe, { asChild: true, children: jsx("span", { className: "text-sm font-medium truncate max-w-[120px]", children: ((_d = a.user) == null ? void 0 : _d.name) || "Unknown User" }) }), jsxs(Me, { children: [jsx("p", { children: ((_e2 = a.user) == null ? void 0 : _e2.name) || "Unknown User" }), jsx("p", { className: "text-xs text-muted-foreground", children: (_f = a.user) == null ? void 0 : _f.email })] })] }) })] }), jsx(it, { className: Ne(a.role), children: a.role })] }) }, a.id);
  }) }), y && jsx($, { variant: "ghost", size: "sm", className: "w-full text-xs h-7", onClick: () => m(true), children: jsxs("span", { className: "flex items-center", children: ["Show ", c.length - C, " More ", jsx(ChevronDown, { className: "ml-1 h-3 w-3" })] }) }), x && c.length > C && jsx($, { variant: "ghost", size: "sm", className: "w-full text-xs h-7", onClick: () => m(false), children: jsxs("span", { className: "flex items-center", children: ["Show Less ", jsx(ChevronUp, { className: "ml-1 h-3 w-3" })] }) }), h.length > 0 && jsxs(Ne$1, { open: v, onOpenChange: M, className: "mt-3", children: [jsxs("div", { className: "flex items-center text-xs text-muted-foreground", children: [jsx("div", { className: "h-px bg-border flex-grow mr-2" }), jsx(pe, { asChild: true, children: jsxs("button", { className: "flex items-center gap-1 px-2 py-1 rounded-sm hover:bg-muted/50 transition-colors", children: [jsxs("span", { children: ["Former Members (", h.length, ")"] }), v ? jsx(ChevronUp, { className: "h-3 w-3" }) : jsx(ChevronDown, { className: "h-3 w-3" })] }) }), jsx("div", { className: "h-px bg-border flex-grow ml-2" })] }), jsx(le, { className: "pt-2", children: jsx("div", { className: "space-y-1.5", children: P.map((a) => {
    var _a, _b, _c, _d, _e2, _f;
    return jsx("div", { children: jsxs("div", { className: "relative flex items-center justify-between group py-1.5", children: [jsx("div", { className: "absolute inset-0 rounded-md bg-muted/30 pointer-events-none" }), jsxs("div", { className: "flex items-center gap-2 z-10", children: [jsxs("div", { className: "relative", children: [jsx(qe, { className: "h-7 w-7", children: ((_a = a.user) == null ? void 0 : _a.image) ? jsx("img", { src: a.user.image, alt: ((_b = a.user) == null ? void 0 : _b.name) || "User", className: "filter grayscale" }) : jsx("div", { className: "bg-gray-200 h-full w-full flex items-center justify-center text-gray-700 text-xs", children: (((_c = a.user) == null ? void 0 : _c.name) || "U").charAt(0) }) }), jsx("div", { className: "absolute -bottom-1 -right-1 rounded-full w-3 h-3 bg-muted border border-background flex items-center justify-center", children: jsx("span", { className: "text-[8px]", children: "\xD7" }) })] }), jsx(dt, { children: jsxs(_e, { children: [jsx(Oe, { asChild: true, children: jsx("span", { className: "text-sm font-medium truncate max-w-[120px] text-muted-foreground", children: ((_d = a.user) == null ? void 0 : _d.name) || "Unknown User" }) }), jsxs(Me, { children: [jsx("p", { children: ((_e2 = a.user) == null ? void 0 : _e2.name) || "Unknown User" }), jsx("p", { className: "text-xs text-muted-foreground", children: (_f = a.user) == null ? void 0 : _f.email }), jsx("p", { className: "text-xs text-red-500 mt-1", children: "No longer a member" })] })] }) })] }), jsx(it, { className: `${Ne(a.role)} opacity-70`, children: a.role })] }) }, a.id);
  }) }) })] })] });
}
const qt = function() {
  var _a, _b, _c, _d, _e2;
  const { projectId: x } = Ln.useParams(), [m, v] = useState(false), [M, C] = useState(true), [b, c] = useState(""), [h, w] = useState(""), [j, P] = useState(false), [D, y] = useState(false), [a, k] = useAtom(St), o = useSetAtom(Le), n = Ln.useRouteContext(), [i] = useState(n.userId), r = a.flatMap((s) => s.projects || []).find((s) => s.id === x), d = a.find((s) => s.id === (r == null ? void 0 : r.teamspaceId));
  useEffect(() => {
    r && C(false);
  }, [r]);
  const T = useNavigate();
  if (M) return jsxs("div", { className: "flex-1 flex flex-col h-full", children: [jsx("div", { className: "h-1.5 bg-gray-200 w-full" }), jsxs("div", { className: "p-6 max-w-5xl mx-auto w-full flex-1", children: [jsxs("div", { className: "mb-8 relative", children: [jsx(be, { className: "h-10 w-60 mb-2" }), jsx(be, { className: "h-4 w-full max-w-md mb-4" })] }), jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-6", children: [jsxs("div", { className: "md:col-span-2 space-y-6", children: [jsx("div", { children: jsx(be, { className: "h-32 w-full mb-6" }) }), jsxs("div", { children: [jsx("div", { className: "flex justify-between items-center mb-4", children: jsx(be, { className: "h-8 w-40" }) }), jsxs("div", { className: "space-y-3", children: [jsx(be, { className: "h-16 w-full" }), jsx(be, { className: "h-16 w-full" })] })] })] }), jsxs("div", { className: "space-y-6", children: [jsx(be, { className: "h-60 w-full" }), jsx(be, { className: "h-60 w-full" })] })] })] })] });
  if (!r) return jsxs("div", { className: "flex-1 flex flex-col items-center justify-center p-8 text-center", children: [jsx("h2", { className: "text-2xl font-bold mb-4", children: "Project not found" }), jsx("p", { className: "text-muted-foreground mb-6", children: "The project you're looking for doesn't exist or you don't have access to it." }), d ? jsx($, { variant: "outline", asChild: true, children: jsx(Link, { to: "/teamspace/$teamspaceId", params: { teamspaceId: d.id }, children: "Back to Teamspace" }) }) : jsx($, { variant: "outline", asChild: true, children: jsx(Link, { to: "/chat", children: "Go to Chat" }) })] });
  const S = async (s = "") => {
    var _a2;
    if (m) return;
    v(true);
    const f = ro({ currentUserId: i, project: r, teamspace: d });
    try {
      const { data: I } = await Te().chatrooms.createChatroom({ chatroomCreatorId: i, chatroomMembers: f, isPrivate: false, projectId: r.id, name: s.trim() || "New Chat Conversation", teamIds: [], type: "PROJECT_CHATROOM" });
    ...
    if (!(!b.trim() || m)) try {
      v(true);
      const s = ro({ currentUserId: i, project: r, teamspace: d }), { data: f } = await Te().chatrooms.createChatroom({ chatroomCreatorId: i, chatroomMembers: s, isPrivate: false, projectId: r.id, name: "New Chat Conversation", teamIds: [], type: "PROJECT_CHATROOM" });
